...
const priorityPages = {
  ...
  "/media-kit": {
    ...
    description: "Public media kit for @ikramrana.ai — ~30K followers, ~2.3M views / 30 days. AI tool and automation Reels for founders and builders. Email ir@ikramrana.com for partnership options and pricing.",
    ...
    faq: [
      ["Same as booking a consult?", "No. Partnerships via ir@ikramrana.com. Consults separate."],
      ["Where are the rates?", "Partnership options and pricing overview, emailed after product + timing."],
      ["What to include?", "Product URL, job-to-be-done, launch window, one-off or monthly."],
    ],
  },
  "/privacy": {
...
function staticShell(details, canonicalUrl) {
  ...
  if (isMediaKit) {
    ...
    return `<div data-static-prerender="true" data-media-kit-prerender="true" style="min-height:100vh;background:#080808;color:#F5F5F2;font-family:'Source Sans 3',system-ui,sans-serif">
      <header style="border-bottom:1px solid #2B2B28;background:#080808"><nav aria-label="Primary navigation" style="max-width:1120px;margin:auto;padding:20px;display:flex;gap:22px;flex-wrap:wrap;align-items:center"><a href="/" style="font-family:'Playfair Display',Georgia,serif;font-size:1.1rem;font-weight:600;color:#F5F5F2;text-decoration:none;margin-right:auto">Ikram Rana</a><a href="/solutions" style="color:#C9C9C3">Solutions</a><a href="/blog" style="color:#C9C9C3">Blog</a><a href="/about" style="color:#C9C9C3">About</a><a href="/media-kit" aria-current="page" style="color:#B9EB38">Media kit</a><a href="/contact" style="color:#C9C9C3">Contact</a></nav></header>
      <main id="main-content" style="max-width:1120px;margin:auto;padding:clamp(64px,9vw,112px) 20px"><p style="font-family:'JetBrains Mono',monospace;text-transform:uppercase;letter-spacing:.2em;color:#B9EB38;font-size:.7rem">${escapeHtml(eyebrow)}</p><h1 style="max-width:880px;font-family:'Playfair Display',Georgia,serif;font-size:clamp(3rem,8vw,6.8rem);font-weight:500;line-height:.96;letter-spacing:-.04em;margin:24px 0 32px">${escapeHtml(details.heading)}</h1>${mediaParagraphs}<p style="margin-top:36px;display:flex;gap:18px;align-items:center;flex-wrap:wrap"><a href="${escapeHtml(ctaHref)}" style="display:inline-block;background:#B9EB38;color:#080808;padding:14px 20px;text-decoration:none;font-weight:700">${escapeHtml(ctaLabel)}</a><a href="https://v2.stan.store/ikramrana" style="color:#C9C9C3">AI consult (separate)</a></p><div style="margin-top:72px;border-top:1px solid #343430;border-bottom:1px solid #343430;padding:22px 0;color:#ADADA6;font-family:'JetBrains Mono',monospace;font-size:.72rem;text-transform:uppercase;letter-spacing:.08em">~30K followers · ~2.3M views / 30 days · walkthrough Reels · founders / operators / builders</div></main>
